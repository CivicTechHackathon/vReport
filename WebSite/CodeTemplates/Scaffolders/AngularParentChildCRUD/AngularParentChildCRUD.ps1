...
$ChildRelationPluralForController = Get-PluralizedWord $ChildRelation;
...
Add-ProjectItemViaTemplate $outputPath -Template "_Detail" -Model @{
	...
	MainViewDataTypeName = $foundModelType.Name;
	RelatedEntities = $relatedChildEntities;
	ChildRelationPlural = $ChildRelationPluralForController;
} -SuccessMessage "Added $ViewName view at '{0}'" -TemplateFolders $TemplateFolders -Project $Project -CodeLanguage $CodeLanguage -Force:$Force
